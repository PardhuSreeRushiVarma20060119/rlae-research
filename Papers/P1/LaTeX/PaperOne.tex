...
This section presents the empirical results of the M-series experiments. All results are reported using the fixed prompt set and evaluation protocol defined in Section~\ref{sec:exp_setup}. The focus is on behavioral divergence, recoverability, identity leakage, and robustness under different adaptation paradigms.

\subsection{Reversibility Under Behavioral Adaptation (RLAE)}
\label{sec:results_rlae}

We first evaluate reversible behavioral adaptation implemented via runtime-mounted LoRA adapters. After adaptation, behavioral parameters are unloaded and the model is re-evaluated against the original baseline identity.

\begin{table}[htbp]
\centering
\caption{Post-reset divergence under reversible behavioral adaptation (RLAE).}
\label{tab:rlae_recovery}
\begin{tabular}{c S[table-format=1.4] c}
\toprule
$\epsilon$ & {Post-reset KL} & Recovery \\
\midrule
0.0 & 0.0599 & Partial \\
0.2 & 0.0468 & Partial \\
$\geq 0.6$ & 0.0000 & Exact \\
\bottomrule
\end{tabular}
\end{table}


As shown in Table~\ref{tab:rlae_recovery}, unloading the behavioral adapters yields exact recovery of the baseline model once sufficient elimination is applied. For $\epsilon \geq 0.6$, the post-reset KL divergence collapses to numerical zero, indicating complete restoration of the core model identity.

\FloatBarrier
\subsection{Structural Variance and Robustness (SVAR)}
\label{sec:results_svar}

We evaluate robustness by applying bounded perturbations to the adaptive components and measuring output divergence.

\begin{table}[htbp]
\centering
\caption{Structural Variance Analysis (SVAR) under bounded perturbations.}
\label{tab:svar}
\begin{tabular}{l c S[table-format=1.3]}
\toprule
Perturbation & Intensity & {KL Divergence} \\
\midrule
Layer Dropout & 0.25 & 0.001 \\
Weight Decay & 0.10 & 0.002 \\
Gaussian Noise & 0.01 & 0.015 \\
\midrule
Adversarial Tensor & 0.05 & 0.265 \\
\bottomrule
\end{tabular}
\end{table}


Table~\ref{tab:svar} shows that reversible behavioral adaptation exhibits high robustness to stochastic perturbations such as dropout and noise. In contrast, adversarial perturbations induce significantly larger divergence, defining the upper bound of the system’s structural stability envelope.

\FloatBarrier
\subsection{Irreversibility of Weight-Based Adaptation}
\label{sec:results_weight}

We next evaluate weight-based adaptation by directly modifying the core parameter set $\theta$ using increasing mutation intensities, followed by post-hoc reset attempts.

\begin{table}[htbp]
\centering
\caption{Irreversibility under increasing weight mutation intensity.}
\label{tab:mutation}
\begin{tabular}{c S[table-format=2.3] l}
\toprule
$\alpha$ & {Post-reset KL} & Output Condition \\
\midrule
...
Table~\ref{tab:mutation} demonstrates a monotonic increase in post-reset divergence with mutation intensity. Even minimal perturbations introduce persistent behavioral deviation, confirming the structural irreversibility of weight-based adaptation.

\FloatBarrier
\subsection{Identity Leakage and Residual Scarring}
\label{sec:results_ils}

To capture localized behavioral residue after reset, we compute prompt-level Identity Leakage Scores (ILS). Reversible behavioral adaptation yields near-zero ILS across all prompts, indicating preservation of functional identity. In contrast, weight-based adaptation produces elevated ILS values on a subset of prompts, even when global divergence appears partially reduced.

These localized deviations represent \emph{identity scars}: persistent behavioral artifacts that survive reset attempts and are not fully captured by aggregate divergence metrics alone.

\FloatBarrier
\subsection{Effect of Model Scale}
\label{sec:results_scale}

Finally, we evaluate adaptation behavior across multiple model scales within the same architectural family.

\begin{table}[htbp]
\centering
\caption{Recoverability factor across model scales.}
\label{tab:scale}
\begin{tabular}{c l S[table-format=3.0]}
\toprule
Model Scale & Adaptation Method & {RF (\%)} \\
\midrule
\multirow{2}{*}{1.5B} & RLAE & 100 \\
                     & Weight Mutation & 30 \\
\midrule
\multirow{2}{*}{3B}   & RLAE & 100 \\
                     & Weight Mutation & 40 \\
\midrule
\multirow{2}{*}{7B}   & RLAE & 100 \\
                     & Weight Mutation & 10 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:scale} shows that recoverability under weight mutation degrades with increasing model scale, while reversible behavioral adaptation remains invariant. This scale-dependent divergence further supports the claim that irreversibility is a structural property of shared-parameter adaptation.

\FloatBarrier
\subsection{Summary of Experiments}
\label{sec:results_exp}

Across all conducted experiments, a clear and consistent structural asymmetry emerges between weight-based adaptation and reversible behavioral adaptation. When adaptive behavior is confined to structurally isolated, runtime-controlled components, the system exhibits exact and deterministic rollback: unloading the behavioral parameters reliably restores the model to its original baseline behavior, with post-reset divergence collapsing to numerical zero across prompts, model scales, and evaluation runs. This behavior is not an artifact of optimization quality or training budget, but a direct consequence of the architectural separation between core identity parameters and adaptive behavior.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/fig_exact_rollback.pdf}
    \caption{Exact rollback via behavioral elimination. Mean post-reset KL divergence collapses to numerical zero once the elimination rate exceeds a critical threshold $\epsilon^\ast$.}
    \label{fig:elim}
\end{figure}
...
\begin{table}[H]
    ...
    \vspace{5pt}
    \caption{Post-reset KL divergence under increasing behavioral elimination rate $\epsilon$.}
    \label{tab:elim}
\end{table}

\FloatBarrier

Figure~\ref{fig:elim} demonstrates the effect of progressive behavioral elimination on post-reset divergence under reversible behavioral adaptation. As the elimination rate $\epsilon$ increases, the mean post-reset KL divergence decreases monotonically, reflecting the systematic removal of adaptive behavioral capacity from the model. This behavior indicates that adaptation-induced divergence is localized within the removable behavioral component rather than entangled with the frozen core parameters.


% =========================================================
\subsection{Structural Irreversibility under Weight Mutation}
\label{sec:weight}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/fig_structural_irreversibility.pdf}
    \caption{Structural irreversibility under weight mutation. Post-reset divergence increases monotonically with mutation intensity $\alpha$ and does not recover.}
    \label{fig:weight}
\end{figure}
...
% =========================================================
\subsection{Recoverability Collapse: Weight-Based vs Reversible Adaptation}
\label{sec:rf}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/fig_structural_recov.pdf}
    \caption{Structural collapse of recoverability. Weight-based adaptation exhibits irreversible scarring, while reversible behavioral adaptation achieves exact rollback.}
    \label{fig:rf}
\end{figure}
...
\begin{table}[H]
    ...
    \vspace{5pt}
    \caption{Recoverability factor (RF) across adaptation mechanisms.}
    \label{tab:rfc}
\end{table}

\FloatBarrier

Figure~\ref{fig:rf} contrasts the recoverability of weight-based adaptation with reversible behavioral adaptation using the recoverability factor (RF). Across all evaluated settings, weight-based adaptation yields near-zero recoverability, indicating that post-reset behavior remains closer to the adapted state than to the original baseline. This behavior is consistent with persistent identity scarring observed in the post-reset divergence measurements.

In contrast, reversible behavioral adaptation consistently achieves $\mathrm{RF}=100\%$, corresponding to exact restoration of baseline behavior after rollback. This stark separation between adaptation paradigms highlights a structural collapse of recoverability in shared-parameter learning. Importantly, this collapse is not sensitive to task type, prompt distribution, or training budget, reinforcing the conclusion that recoverability is determined by the structural locus of adaptation rather than by optimization quality or regularization strategy.

% =========================================================
\subsection{Baseline Identity Stability Across Sprints}
\label{sec:baseline}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/fig_baseline_entropy.pdf}
    \caption{Baseline output entropy across sprints. No systematic drift is observed.}
    \label{fig:baseline}
\end{figure}
...
Figure~\ref{fig:baseline} reports the output entropy of the frozen base model evaluated across all experimental sprints. Both the mean entropy and its variance remain stable over time, with no systematic trend or drift observed. This stability confirms that the base model identity remains unchanged across the experimental pipeline.

By ruling out baseline identity drift, these results eliminate a key confounding factor in the interpretation of post-reset divergence. The observed differences between adaptation paradigms therefore cannot be attributed to changes in the underlying model or evaluation distribution, but must instead arise from the structural properties of the adaptation mechanisms themselves.

In simple terms, this experiment verifies that the base model itself remains stable throughout all experimental runs. The output entropy of the frozen model does not change across sprints, indicating that neither the model nor the evaluation setup drifts over time. This confirms that the baseline behavior is consistent and reliable.

As a result, any divergence observed after adaptation and reset cannot be explained by accidental changes in the base model or data distribution. Instead, the differences we observe directly reflect the effects of the adaptation mechanisms being tested. This validation step ensures that our conclusions about reversibility and irreversibility are not influenced by hidden baseline instability.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/fig_recoverability_model_scale.pdf}
...
This was was supported in part by \ldots
