...
We conduct all experiments using a single pretrained neural model/ large language model with parameters $\theta$, treated as a fixed core identity throughout evaluation. The specific architecture and scale of the model are not central to our claims; instead, we require only that the model supports both direct weight updates and parameter-isolated behavioral adaptation.
...
We emphasize that no metric introduced in this work is optimized during training; all metrics are computed post hoc for evaluation and analysis only.
