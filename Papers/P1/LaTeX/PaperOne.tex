...
In addition to KL divergence, we compute Jensen–Shannon (JS) divergence as a symmetric and bounded confirmation metric. While KL captures directional distributional drift, JS provides a numerically stable measure of distributional similarity that is less sensitive to tail amplification. The use of both metrics ensures that recoverability claims are not artifacts of KL asymmetry or support mismatch.

It is important to note that divergence is evaluated with respect to a fixed prompt distribution and finite-precision numerical representation. Thus, exact recovery in this work refers to distributional equivalence under the evaluation protocol rather than strict parameter-level equality. Zero divergence indicates that no observable behavioral deviation remains within the measurement resolution.
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\linewidth}
		...
		\caption{Mean post-reset KL divergence vs. elimination rate $\varepsilon$.}
		\label{fig:elim_kl}
	\end{subfigure}
	...
	\caption{
		...
		(Right) Mean post-reset KL divergence exhibits a sharp threshold collapse at 
		$\varepsilon^\ast = 0.6$, marking the transition from partial to exact recovery.
	}
...
We define exact recovery as the regime in which both KL and JS divergence fall below $10^{-6}$, corresponding to numerical precision limits.
...
Table~\ref{tab:rfc} contrasts the recoverability of weight-based adaptation with reversible behavioral adaptation using the recoverability factor (RF). Across all evaluated settings, weight-based adaptation yields near-zero recoverability, indicating that post-reset behavior remains closer to the adapted state than to the original baseline. This behavior is consistent with persistent identity scarring observed in the post-reset divergence measurements.

The collapse of both KL and JS divergence to numerical zero under behavioral unload, contrasted with persistent divergence under shared-parameter mutation, indicates that reversibility is a structural property of parameter separation rather than an artifact of optimization quality.
