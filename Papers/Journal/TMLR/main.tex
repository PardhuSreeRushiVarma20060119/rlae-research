...
\begin{abstract}
The neural models are usually updated or modified by adjusting the parameters of the model, such as fine-tuning and alignment based on reinforcement learning, although these methods are helpful in the traditional short-term optimization, it may lead to irreversible modifications to the basic behaviors of the neural \& machine learning models, such as making them poor at reasoning or losing their basic capabilities and identity. In our work, we investigate the idea of \emph{structural irreversibility} as a restriction on modifying the shared parameters of a model. We have done some experiments, and we found that the direct modification of the parameters of the model may confuse the objectives of a specific task with the general parameters of the model, so that the model behaves in a different manner in a way that cannot be reversed by any method that can be realistically applied. On the other hand, we investigated a method to modify the model in a reversible manner where the things learned by the model are separated from the general model and do not impact or modify it. We found that this helps the model to learn to adapt to a task but still be able to switch back to its original state with little to no difference in behavior. Finally, we were able to propose a way to measure the capability of a model to reverse changes. we formalize this distinction by proposing \emph{recoverability} as a clear evaluation metric for adaptive systems. we propose a way to measure the robustness of a model when it is changed in a controlled fashion. The idea of being able to reverse changes in a model is not as clear-cut in existing traditional machine learning \& deep learning approaches. It is very important in the development of neural systems that are safe and can be controlled and will last for a very long time. Neural models and large language models are what we are dealing with in this scenario and the idea of being able to reverse changes is part of developing them to function well and safely and effectively. and Large language models need to be able to adapt to new tasks without losing their original capabilities, and neural models need to be able to change in a safe and controlle manner.
\end{abstract}
...
\section{Introduction}
	...
	This model adjustment process also puts a significant amount of stress on the parts of the model that keep it running. It can cause the model to perform inefficiently in its reasoning, forget what it already knew or learned, or act differently because the weights are changing too quickly. These problems are difficult to identify and solve after the model has been put into use, and the intentions of the model can become unclear due to the information it has learned.
	...
	The key observation here is that the degree to which the adaptation is reversible depends on the level of the model on which the adaptation is being represented. When the adaptation is represented at the level of the parameters, the adaptation is distributed because the parameters represent multiple abstractions simultaneously. This means that the task-specific objectives become conflated with the basic representations, a phenomenon that is well-documented in the literature on continual learning \cite{kirkpatrick2017ewc,delange2021continual}. This means that the adaptation is not easily reversible, and the adaptation cannot be easily undone \cite{kaushik2021rmn}.
	
	With the adaptive behavior isolated from the base model, by breaking down the learned behavior into removable chunks while keeping the base model parameters constant, the results from previous studies \cite{kaushik2021rmn,lanzillotta2023isolation} have shown a significantly improved retention and recoverability. Since the base model parameters are not modified, the rollback process does not require approximations, retraining, or checkpointing. It can simply be achieved by removing or turning off the behavioral component.
	...
	must solve an optimization problem in parameter space.
	In general,
	
	\[
	D_{\mathrm{KL}}(f_\theta \,\|\, \mathcal{R}(f_{\theta'})) > 0.
	\]
	
	\textit{Proof sketch:}
	Direct weight mutation entangles adaptation within the global parameter manifold.
	Without an exact snapshot, recovering $\theta$ from $\theta'$ requires
	approximate inversion in high-dimensional space. 
	Because neural network parameterizations are non-linear and many-to-one
	with respect to behavioral mappings, the inverse mapping is not uniquely
	recoverable from outputs alone.
	Thus, exact restoration of $p_\theta(y \mid x)$ is not guaranteed,
	implying strictly positive divergence under finite-precision evaluation.
	\hfill $\square$
